...
public class MedicineQuery {
	...
	public static void medicen_stock(Connection conn, Statement stmt, String chemistID) {
		...
		String p_num = pharmacy_num(conn,stmt,chemistID);
		...
		try {
			...
			String sql = "select m.m_number, m.stock "
	...
	public static String pharmacy_num(Connection conn, Statement stmt, String chemistID) {
		...
		try {
			...
			while (rs.next()) {
				...
				p_num = rs.getString(1);//약 번호 받기 위해 약 업데이트에 필요
	...
	public static void case_search(Connection conn, Statement stmt, String Id) {
		...
		try {
			...
			System.out.println("\n-------------------------------------");
			System.out.println("-------- [ 병력 기록 조회 결과 ] --------\n");
			int i=0;
			while(rs.next()) {
				SimpleDateFormat checkDate = new SimpleDateFormat("yyyy-MM-dd");
	...
	public static void main(String[] args) {
		...
		case 1:
			...
			while(true) {
				...
				System.out.println("5. 병력 기록 추가");
				System.out.println("6. 병력 기록 조회");
